...
data {
  ...
  vector[N] log_gest;    // 
  vector[N] log_weight;  // 
  vector[N] preterm;     // preterm variable
}
transformed data {
...
parameters {
  vector[2] beta;           // coefs
  vector[2] gamma;      // coef interaction
  real<lower=0> sigma;  // error sd for Gaussian likelihood
}
model {
  // Log-likelihood
  target += normal_lpdf(log_weight | beta[1] + beta[2] * log_gest + gamma[1] * preterm + gamma[2] * inter, sigma);
...
generated quantities {
  ...
  for (n in 1:N) {
    real log_weight_hat_n = beta[1] + beta[2] * log_gest[n] + gamma[1] * preterm[n] + gamma[2] * log_gest[n];
