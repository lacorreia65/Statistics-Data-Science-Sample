...
def train_log_regression(images, labels, learning_rate, max_iter):
    ...
    while (not converged and (it < max_iter)):
        ...
        # Calculate the prediction - This info is also self-cintained in lsmax
        # psmax = predict(lsmax) # v.FINAL_3
        
        # Calculate the Cross Entropy over the prediction
        # cent = cross_ent(psmax, labels) # vFINAL_3
        ...
        # grad_history[it] = np.linalg.norm(grad)  # v.FINAL_4
        grad_history[it] = (np.linalg.norm(grad)**2)/(N_data*K_data) # Calculated l2-Norm
    ...
    plt.plot(cross_ent_history[:(it-1)]); 
    ...
    plt.title('Training Accuracy History | Learning rate='+
              str(learning_rate)+' / Max-Iter='+str(max_iter), fontsize=12)
    plt.plot(accuracy_history[:(it-1)]); 
    plt.xlabel('Iteration')
    plt.ylabel('Accuracy')
    # plt.show()
    ...
    # Save Gradient l2-Norm for later reference
    plt.figure(figsize=(10,7))  
    plt.title('Gradient\'s Normalized Average Squared l2-Norm | Learning rate='+
              str(learning_rate)+' / Max-Iter='+str(max_iter), fontsize=12)
    plt.plot(grad_history[:(it-1)]); 
    plt.xlabel('Iteration')
    plt.ylabel('l2-Norm')
    # plt.show()
    plt.savefig('Grad-l2Norm-GD.png')
    plt.close()    
