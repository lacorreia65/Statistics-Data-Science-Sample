...
After running a disgnostics using \texttt{arima()} and diagnostics with \texttt{tsdiag()}\footnote{\label{q5ft01}We know that the recommended option to check for model consistency would be use the \texttt{sarima()} function but in this case, we are just checking a preliminar model, with no major issue in our approach.}. we can then see that residuals seems to be uncorrelated and that there are not further structures in residuals that may require our attention, so we can see this could be a valid starting model for further analysis. 
